...
_DDX_PROMPT = (
    ...
    'приложенным документам (результаты анализов/обследований), внесённым '
    'пациентом вручную результатам (concept=analysis, поле result) и свободным '
    'комментариям пациента (concept=note, поле text) предложи '
...
_WORKUP_PROMPT = (
    ...
    'предложи, какие анализы и обследования стоит пройти пациенту, чтобы уточнить '
    'предположительный диагноз. Только обоснованное. Ранжируй по соотношению '
    'диагностической ценности и доступности: при сопоставимой ценности сначала '
    'простые, дешёвые и выполнимые самостоятельно (например, позиционные пробы, '
    'дневник давления), затем лабораторные, затем дорогие/инструментальные. '
    'test — название анализа/обследования; reason — зачем (что подтвердит/исключит); '
    'self=true, если пациент может выполнить сам дома без медработника и '
    'спецоборудования — тогда reason должен включать короткую инструкцию выполнения. '
    'code — стабильный идентификатор анализа: латиница, snake_case, канонично и '
    'повторяемо (тот же анализ при пересчёте получает тот же code), '
    'например dix_hallpike, cbc, brain_mri.'
)
    ...
    'properties': {
        'tests': {
            ...
            'items': {
                'type': 'OBJECT',
                'properties': {
                    'code': {'type': 'STRING'},    # стабильный id: dix_hallpike, cbc
                    'test': {'type': 'STRING'},
                    'reason': {'type': 'STRING'},
                    'self': {'type': 'BOOLEAN'},   # выполним пациентом дома
                },
                'required': ['code', 'test', 'reason', 'self'],
            },
        },
    },
    'required': ['tests'],
