...
export default function Diary() {
  ...
  const closeEditor = () => {
    setCode(''); setValue(''); setContext(''); setNote('')
    setEditor(null)
  }
  ...
  const addVital = async () => {
    ...
    try {
      ...
      await addDiaryEntry({
        ...
        value: { value: value.trim(), unit: ui(UNITS[code] ?? ''),
      ...
      <section>
        ...
        {editor === 'vital' && (
          <div className="card diary-editor">
            <div className="inline">
              <select value={code} onChange={(e) => { setCode(e.target.value); setContext('') }}>
              ...
              <input autoFocus placeholder={ui('значение')} value={value}
          ...
          {entries.map((p) => {
            ...
            return (
              <li key={p.id} className="row-link">
                <span>{isNote ? String(val.text) : p.name || p.code}
                  {!isNote && val.context && <span className="muted"> · {ui(GLUCOSE_CTX[val.context] ?? '')}</span>}</span>
                {!isNote && <b>{String(val.value ?? '')} {String(val.unit ?? '')}</b>}
